...
func TestConnectivityVNetWithNAT1To1RuleOptions(t *testing.T) {
	defer test.CheckRoutines(t)()
	...
	t.Run("host candidate mapping with options", func(t *testing.T) {
		...
		agent0Opts := []AgentOption{
			...
			WithNAT1To1Rules(NAT1To1Rule{
				PublicIPs:       []string{vnetGlobalIPA},
...
func TestConnectivityVNetNAT1To1SharedFoundation(t *testing.T) {
	...
	agent, err := NewAgentWithOptions(
		...
		WithMulticastDNSMode(MulticastDNSModeDisabled),
		WithNAT1To1HostIPs(vnetGlobalIPA),
		WithNAT1To1SrflxIPs(vnetGlobalIPA),
	)
	require.NoError(t, err)
	defer func() {
