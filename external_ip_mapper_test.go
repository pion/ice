...
func assertExternalIPStrings(
	t *testing.T, mapper *addressRewriteMapper, candidateType CandidateType, localIP string, expected ...string,
) {
	t.Helper()
...
func assertExternalIPError(
	t *testing.T, mapper *addressRewriteMapper, candidateType CandidateType, localIP string, expected error,
) {
	t.Helper()

	_, matched, _, err := mapper.findExternalIPs(candidateType, localIP)
	assert.True(t, matched)
	...
	cases := []struct {
	...
	}{
		{
			...
			expectMapper: func(t *testing.T, mapper *addressRewriteMapper) {
				...
				assert.NotNil(t, mapper)
				assertExternalIPStrings(t, mapper, CandidateTypeHost, "10.0.0.1", "1.2.3.4")
				assertExternalIPStrings(t, mapper, CandidateTypeHost, "2001:db8::10", "2001:db8::1")
			},
		},
		{
		...
		},
		{
			...
			expectMapper: func(t *testing.T, mapper *addressRewriteMapper) {
				...
				assertExternalIPStrings(t, mapper, CandidateTypeHost, "10.0.0.1", "2200::1")
			},
		},
		{
			...
			expectMapper: func(t *testing.T, mapper *addressRewriteMapper) {
				...
				assertExternalIPStrings(t, mapper, CandidateTypeHost, "fe80::1", "1.2.3.4")
			},
		},
		{
