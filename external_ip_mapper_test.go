...
func TestExternalIPMapper(t *testing.T) {
	t.Run("validateIPString", func(t *testing.T) {
		...
		assert.True(isIPv4, "should be true")
		assert.Equal("1.2.3.4", ip.String(), "should be true")

		ip, isIPv4, err = validateIPString("2601:4567::5678")
		assert.NoError(err, "should succeed")
		assert.False(isIPv4, "should be false")
		assert.Equal("2601:4567::5678", ip.String(), "should be true")
	...
	t.Run("newExternalIPMapper", func(t *testing.T) {
		...
		assert.NoError(err, "should succeed")
		assert.NotNil(m, "should not be nil")
		assert.Equal(CandidateTypeHost, m.candidateType, "should match")
		assert.NotNil(m.ipv4Mapping.ipSole)
		assert.Nil(m.ipv6Mapping.ipSole)
		assert.Equal(0, len(m.ipv4Mapping.ipMap), "should match")
		assert.Equal(0, len(m.ipv6Mapping.ipMap), "should match")
		...
		assert.NoError(err, "should succeed")
		assert.NotNil(m, "should not be nil")
		assert.Equal(CandidateTypeServerReflexive, m.candidateType, "should match")
		assert.NotNil(m.ipv4Mapping.ipSole)
		assert.Nil(m.ipv6Mapping.ipSole)
		assert.Equal(0, len(m.ipv4Mapping.ipMap), "should match")
		assert.Equal(0, len(m.ipv6Mapping.ipMap), "should match")
	...
	t.Run("newExternalIPMapper with explicit local IP", func(t *testing.T) {
		...
		assert.Nil(m.ipv4Mapping.ipSole)
		assert.Nil(m.ipv6Mapping.ipSole)
		assert.Equal(1, len(m.ipv4Mapping.ipMap), "should match")
		assert.Equal(0, len(m.ipv6Mapping.ipMap), "should match")
	...
	t.Run("findExternalIP without explicit local IP", func(t *testing.T) {
		...
		assert.NoError(err, "should succeed")
		assert.Equal("1.2.3.4", extIP.String(), "should match")

		// Find external IPv6
		extIP, err = m.findExternalIP("fe80::0001") // Use '0001' instead of '1' on purpose
		assert.NoError(err, "should succeed")
		assert.Equal("2200::1", extIP.String(), "should match")
	...
	t.Run("findExternalIP with explicit local IP", func(t *testing.T) {
		...
		assert.NoError(err, "should succeed")
		assert.Equal("1.2.3.5", extIP.String(), "should match")
		...
		assert.NoError(err, "should succeed")
		assert.Equal("2200::2", extIP.String(), "should match")
	...
	t.Run("findExternalIP with empty map", func(t *testing.T) {
		...
		assert.NoError(err, "should succeed")
		assert.Equal("fe80::1", extIP.String(), "should match")
		...
		extIP, err = m.findExternalIP("10.0.0.1")
		assert.NoError(err, "should succeed")
		assert.Equal("10.0.0.1", extIP.String(), "should match")
	})
}
