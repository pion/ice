...
// addresses are replaced with the supplied external IPs. This covers mixed
// topologies such as NAT64/CLAT environments where IPv6-only hosts need
// deterministic IPv4 representations.
...
// local address, the first matching rule wins. Within a rule, matches pinned
// to a specific `Local` address are preferred over `CIDR` scopes, which are
// preferred over the per-family catch-all entry. Overlapping rules in the same
// scope are logged as warnings (not errors) to avoid breaking existing configs
// when users adjust rule sets; prefer consolidating to a single rule per scope.
