...
// Package main provides a a very CLI that prints the host and srflx addresses
// produced by different NAT 1:1 rule configurations. It is designed to be run
...
func main() {
	...
	fmt.Println("NAT 1:1 rule demonstration client")
...
func buildMultiNetworkScenario() scenario {
	...
	return scenario{
		...
		NATRules: []ice.NAT1To1Rule{
			{
				PublicIPs: []string{
					fmt.Sprintf("%s/%s", publicBlue, localBlue),
					fmt.Sprintf("%s/%s", publicGreen, localGreen),
				},
				AsCandidateType: ice.CandidateTypeHost,
				Networks:        []ice.NetworkType{ice.NetworkTypeUDP4, ice.NetworkTypeTCP4},
			},
			{
				PublicIPs:       []string{globalFallback},
				AsCandidateType: ice.CandidateTypeHost,
			},
		},
	}
...
func buildSrflxScenario() scenario {
	...
	return scenario{
		...
		NATRules: []ice.NAT1To1Rule{
			{
				PublicIPs: []string{
					fmt.Sprintf("%s/%s", srflxPrimary, srflxLocal),
					srflxSecondary,
				},
			...
			{
				PublicIPs: []string{
					fmt.Sprintf("%s/%s", hostPublic, hostLocal),
				},
				AsCandidateType: ice.CandidateTypeHost,
				Networks:        []ice.NetworkType{ice.NetworkTypeUDP4},
			},
...
func buildScopedCatchAllScenario() scenario {
	...
	return scenario{
		...
		NATRules: []ice.NAT1To1Rule{
			{
				PublicIPs: []string{
					fmt.Sprintf("%s/%s", scopedPublic, scopedLocal),
					catchAll,
				},
				AsCandidateType: ice.CandidateTypeHost,
				CIDR:            scopedCIDR,
...
func runScenario(ctx context.Context, sc scenario) error { //nolint:cyclop
	...
	printRules(sc.NATRules)

	var opts []ice.AgentOption

	if len(sc.NATRules) > 0 {
		opts = append(opts, ice.WithNAT1To1Rules(sc.NATRules...))
	}
...
func printRules(rules []ice.NAT1To1Rule) {
	if len(rules) == 0 {
	...
	fmt.Println("Active NAT 1:1 rules:")
	for idx, rule := range rules {
		...
		fmt.Printf("  %d) %s => [%s]%s\n",
			idx+1,
			candidateType,
			strings.Join(rule.PublicIPs, ", "),
			scope)
	}
}

func describeRuleScope(rule ice.NAT1To1Rule) string {
