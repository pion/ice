...
func (m *TCPMuxDefault) createConn(ufrag string, isIPv6 bool, local net.IP, fromStun bool, logger logging.LeveledLogger) (*tcpPacketConn, error) {
	...
	conns[connKey] = conn
	logger.Errorf("DBG: ufrag: %s, local: %+v, tcpMux add: new conn, fromSTUN: %v", ufrag, conn.LocalAddr(), fromStun) // REMOVE
			...
			for _, conn := range conns {
				...
				if age > time.Hour {
					m.params.Logger.Errorf(
						"DBG: ufrag: %s, local: %+v: tcpMux check: age: %+v, fromSTUN: %v",
						conn.params.Ufrag,
						conn.LocalAddr(),
						age,
						conn.FromSTUN(),
