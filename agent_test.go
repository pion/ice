...
func TestAutomaticRenominationWithVNet(t *testing.T) {
	...
	agent1, err := NewAgent(&AgentConfig{
		NetworkTypes:          []NetworkType{NetworkTypeUDP4},
		MulticastDNSMode:      MulticastDNSModeDisabled,
		Net:                   net0,
		KeepaliveInterval:     &keepaliveInterval,
		CheckInterval:         &checkInterval,
		AutomaticRenomination: true,
		RenominationInterval:  &renominationInterval,
	})
	require.NoError(t, err)
	defer agent1.Close() //nolint:errcheck

	agent1.enableRenomination = true
	agent1.nominationValueGenerator = DefaultNominationValueGenerator()
