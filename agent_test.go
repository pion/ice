...
func TestAddLocalCandidateRejectsNilPacketConn(t *testing.T) {
	...
	require.NoError(t, err)
	require.ErrorIs(t, agent.AddLocalCandidate(candidate, nil), ErrCandidatePacketConnNil)
}

func TestAddLocalCandidateRejectsDuplicateWithoutClosing(t *testing.T) {
	agent, err := NewAgentWithOptions(WithMulticastDNSMode(MulticastDNSModeDisabled))
	require.NoError(t, err)
	defer func() { require.NoError(t, agent.Close()) }()
	...
	require.NoError(t, err)
	packetConn := &localCandidatePacketConn{
		addr: &net.UDPAddr{IP: net.IPv4(192, 0, 2, 12), Port: 5002},
	}

	require.NoError(t, agent.AddLocalCandidate(candidate, packetConn))
	require.ErrorIs(t, agent.AddLocalCandidate(candidate, packetConn), ErrDuplicateCandidate)
...
func TestAddCandidateClosesDuplicate(t *testing.T) {
	...
	require.NoError(t, err)
	firstConn := &localCandidatePacketConn{
		addr: &net.UDPAddr{IP: net.IPv4(192, 0, 2, 13), Port: 5003},
	}
	duplicateConn := &localCandidatePacketConn{
		addr: &net.UDPAddr{IP: net.IPv4(192, 0, 2, 13), Port: 5003},
	}

	require.NoError(t, agent.addCandidate(context.Background(), first, firstConn, false))
	require.NoError(t, agent.addCandidate(context.Background(), duplicate, duplicateConn, false))
...
func TestGetLocalCandidates(t *testing.T) {
	...
	for i := range 5 {
		...
		err = agent.addCandidate(context.Background(), cand, dummyConn, false)
		require.NoError(t, err)
	}
