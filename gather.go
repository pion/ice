...
func (a *Agent) gatherCandidatesSrflxMapped(ctx context.Context, networkTypes []NetworkType, generation uint64) { //nolint:gocognit,cyclop
	var wg sync.WaitGroup
	defer wg.Wait()
...
func (a *Agent) gatherCandidatesSrflxUDPMux(ctx context.Context, urls []*stun.URI, networkTypes []NetworkType, generation uint64) {
	var wg sync.WaitGroup
	defer wg.Wait()
...
func (a *Agent) gatherCandidatesSrflx(ctx context.Context, urls []*stun.URI, networkTypes []NetworkType, generation uint64) {
	var wg sync.WaitGroup
	defer wg.Wait()
...
func (a *Agent) createRelayCandidate(ctx context.Context, ep relayEndpoint, ip net.IP, generation uint64, onClose func() error) error {
	...
	for {
		...
		case <-ticker.C:
			if a.detectNetworkChanges() {
				generationChan := make(chan uint64, 1)
				a.loop.Run(a.loop, func(_ context.Context) {
					newGen := a.bumpGatheringGenerationLocked()
