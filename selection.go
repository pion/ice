...
func (s *controllingSelector) ContactCandidates() {
	...
	case s.agent.getSelectedPair() != nil:
		if s.agent.validateSelectedPair() {
			...
			// Check for automatic renomination if enabled
...
func (s *controllingSelector) checkForAutomaticRenomination() {
	// Check if automatic renomination is enabled
	if !s.agent.automaticRenomination || !s.agent.enableRenomination {
	...
	// Check if enough time has passed since last actual renomination
	// This prevents renominating too frequently
	...
	// Find the best available candidate pair
	...
	// Check if renomination is warranted
