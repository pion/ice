...
func newAgentWithConfig(config *AgentConfig, opts ...AgentOption) (*Agent, error) { //nolint:cyclop
	...
	agent := &Agent{
		...
		enableRenomination: config.AutomaticRenomination, // Auto-enable when automatic renomination is enabled
		nominationValueGenerator: func() NominationValueGenerator {
			if config.AutomaticRenomination {
				return DefaultNominationValueGenerator()
			}
		...
		automaticRenomination: config.AutomaticRenomination,
...
func (a *Agent) shouldRenominate(current, candidate *CandidatePair) bool { //nolint:cyclop
	...
	// Don't renominate to the same pair
	if current.equal(candidate) {
		return false
	}

	// Only renominate to succeeded pairs
	if candidate.state != CandidatePairStateSucceeded {
		return false
	}
	...
	if currentRTT > 0 && candidateRTT > 0 {
		// Convert seconds to Duration for cleaner comparison
